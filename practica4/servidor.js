...
function mostrarBanner() {
  ...
  if (notificaciones.length > 0) {
    console.log("=== AVISOS DE BARRA EN TIEMPO REAL ===");
    const ultimosAvisos = notificaciones.slice(-3);
...
async function procesarPedidoAsincrono(pedido) {
  ...
  notificaciones.push("Pedido " + pedido.id + " ha sido recibido en caja.");
  
  await esperar(4000 + Math.random() * 2000);
  if (pedido.estado === 'Pedido cancelado') return;
  
  pedido.estado = 'Preparando.....';
  notificaciones.push("Pedido " + pedido.id + ": El barista comenzo a preparar tus productos.");
  
  const r = Math.random();
  if (r < 0.2) {
    notificaciones.push("Pedido " + pedido.id + ": Barra saturada. Se demorara un poco mas.");
    await esperar(4000);
  } else if (r < 0.3) {
    pedido.estado = 'Pedido cancelado';
    notificaciones.push("Pedido " + pedido.id + ": ¡Cancelado! Nos quedamos sin insumos basicos.");
    return;
  }
  
  await esperar(5000 + Math.random() * 2000);
  if (pedido.estado === 'Pedido cancelado') return;
  
  pedido.estado = 'Empacado.....';
  notificaciones.push("Pedido " + pedido.id + ": Tus productos estan empacados y listos en barra.");
  ...
  if (Math.random() < 0.06) {
    pedido.estado = 'Pedido cancelado';
    notificaciones.push("Pedido " + pedido.id + ": ¡Cancelado! Ocurrio un accidente al servir la charola.");
  } else {
    pedido.estado = 'Pedido entregado';
    notificaciones.push("Pedido " + pedido.id + ": ¡Entregado! Que disfrutes tu compra.");
  }
}

async function simularPreparacionEnVivo(pedido) {
  mostrarBanner();
  console.log("\n=== SEGUIMIENTO EN VIVO - " + pedido.id + " ===");
  console.log("Cliente: " + pedido.cliente);
  console.log("--------------------------------------------------");
  
  pedido.estado = 'Pedido recibido';
  console.log("--> [Estado] Pedido recibido en caja.");
  console.log("    Barra: [          ] 0%");
  await esperar(3000);
  ...
  console.log("Cliente: " + pedido.cliente);
  console.log("--------------------------------------------------");
  console.log("--> [Estado] Preparando tu cafe.....");
  console.log("    Barra: [===       ] 30%");
  
  const r = Math.random();
  if (r < 0.25) {
    console.log("\n--> ¡Aviso de barra! Hay fila. El barista tardara un poco mas...");
    await esperar(3500);
  } else if (r < 0.4) {
    pedido.estado = 'Pedido cancelado';
    console.log("\n--> ¡Alerta! Nos quedamos sin insumos suficientes.");
    console.log("--> [Estado] Pedido cancelado.");
    console.log("    Barra: [XXXXXXXXXX] 100%");
    console.log("--------------------------------------------------");
    await rl.question("--> Dale ENTER para regresar al menu...");
    return;
  }
  ...
  console.log("Cliente: " + pedido.cliente);
  console.log("--------------------------------------------------");
  console.log("--> [Estado] Empacado y listo en barra.....");
  console.log("    Barra: [=======   ] 70%");
  await esperar(3000);
  ...
  if (Math.random() < 0.08) {
    pedido.estado = 'Pedido cancelado';
    console.log("--> ¡Alerta! Se cayo la charola con tus bebidas en la entrega.");
    console.log("--> [Estado] Pedido cancelado.");
    console.log("    Barra: [XXXXXXXXXX] 100%");
  } else {
    pedido.estado = 'Pedido entregado';
    console.log("--> ¡Tu pedido esta listo! Que lo disfrutes.");
    console.log("--> [Estado] Pedido entregado.");
    console.log("    Barra: [==========] 100%");
  }
    ...
    switch (opCar.trim()) {
      ...
      case '3':
        ...
        if (confirmar.toLowerCase() === 's' || confirmar.toLowerCase() === 'si') {
          ...
          const verSeguimiento = await rl.question("\n¿Desea ver el proceso de barra en tiempo real ahora mismo? (s/n): ");
          if (verSeguimiento.toLowerCase() === 's' || verSeguimiento.toLowerCase() === 'si') {
            await simularPreparacionEnVivo(nuevoPedido);
          } else {
            console.log("\n--> Entendido. Su pedido se procesara en segundo plano.");
            console.log("    Podra ver los avisos de barra en la parte superior del menu principal.");
            procesarPedidoAsincrono(nuevoPedido);
            await new Promise(r => setTimeout(r, 3000));
          }
